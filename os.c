...
/**
 * @brief Get the current time in ticks since the RTOS started.
...
void Start_Periodic(unsigned int tick_offset, unsigned int cycle_offset) {
	...
	/* OK, now we need to find out exactly were our tick offset puts us*/
	while(tick_offset > 0) {
		ticks_remaining = PPP[slot_name_index +1];

		/* if we have more ticks left in this task then we are trying to offset
		 * we can just subtract it */
		if(ticks_remaining >= tick_offset) {
			ticks_remaining -= tick_offset;
			break;
		}else {
			/* we need to move on to the next task */
			tick_offset -= ticks_remaining;
			slot_name_index += 2;
			if(slot_name_index >= PT<<2) {
				slot_name_index = 0;
			}
		}
	}
	...
	/*
	* Restart current tick timer, removing the offset
	*/
	OCR1A = TCNT1 + TICK_CYCLES - cycle_offset;
}
